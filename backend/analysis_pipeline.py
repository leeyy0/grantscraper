...
from models.models import Grant, Initiative, Organisation, Result
...
def upload_pdfs_to_gemini(pdf_paths: list[str]) -> list[types.File]:
    """
...
def run_prelim_scan(initiative_id: int, initial_pdf_dir: str):
    ...
    try:
        ...
        organisation = (
            session.query(Organisation).filter_by(id=initiative.organisation_id).first()
        )
        if not organisation:
...
Return ONLY the integer rating (0-100), nothing else."""
            ...
            try:
                ...
                result = (
                    ...
                    .filter_by(grant_id=grant_id, initiative_id=initiative_id)
                    .first()
                )
                ...
                if result:
                    ...
                    print("Updating existing Result row...")
                else:
                    ...
                    session.add(result)
                    print("Creating new Result row...")

                # 3. Commit transaction to PostgreSQL database
                session.commit()
                print("✓ Saved to database (committed to PostgreSQL)")
...
def generate_shortlist(initiative_id: int, cutoff: int = 60) -> list[int]:
    ...
    try:
        ...
        results = (
            session.query(Result)
            .filter(
                Result.initiative_id == initiative_id, Result.prelim_rating > cutoff
            )
            .all()
        )
        ...
        for result in results:
            ...
            print(
                f"  - Grant {result.grant_id}: {grant.name if grant else 'Unknown'} "
...
def run_deep_analysis(initiative_id: int, deep_pdf_dir: str):
    ...
    try:
        ...
        shortlisted_results = (
            session.query(Result)
            .filter(
                Result.initiative_id == initiative_id, Result.prelim_rating.isnot(None)
            )
            .all()
        )
...
- Return ONLY valid JSON, no markdown formatting, no explanation text"""
            ...
            try:
                ...
                if analysis_data.get("deadline"):
                    try:
                        deadline = datetime.fromisoformat(
                            analysis_data["deadline"].replace("Z", "+00:00")
                        )
                    except Exception:
                        pass
                ...
                if result:
                    ...
                    session.commit()
                    print(
                        "✓ Updated database with deep analysis (committed to PostgreSQL)"
                    )
                else:
...
def export_results_to_frontend(initiative_id: int):
    ...
    try:
        ...
        results = (
            session.query(Result)
            .filter(
                Result.initiative_id == initiative_id, Result.match_rating.isnot(None)
            )
            .all()
        )
...
if __name__ == "__main__":
    ...
    print(
        "(In production, the scraper would run here based on shortlist_for_scraper.json)\n"
    )
