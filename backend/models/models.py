from sqlalchemy import (
    ...
    ForeignKey, Text, JSON, CheckConstraint
)
...
class Result(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint('prelim_rating >= 0 AND prelim_rating <= 100', name='check_prelim_rating'),
        CheckConstraint('match_rating >= 0 AND match_rating <= 100', name='check_match_rating'),
        CheckConstraint('uncertainty_rating >= 0 AND uncertainty_rating <= 100', name='check_uncertainty_rating'),
    )
